  ...
  return (
    <div className="h-full relative overflow-hidden">
        {data?.map((e, i) => (
            <div
            key={i}
            className={`w-full h-full px-10 lg:px-[98px] absolute top-0 left-0 transition-opacity duration-1000 ease-in-out ${i === currentIndex ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}
            style={{
            ...
            >
            <div className="grid grid-cols-1 lg:grid-cols-2 place-items-center h-full w-full">
        ...
        <div className="absolute z-10 left-1/2 bottom-4 transform -translate-x-1/2 flex gap-1">
          {data?.map((_, index) => (
            <button
              key={index}
